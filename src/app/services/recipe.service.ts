...
export class RecipeService {
  ...
  apiURL = 'https://api.edamam.com/api/recipes/v2?type=public&app_id=88ae3946&app_key=731bdd1bb5afc3c6002f651f4059918c&mealType=Breakfast&mealType=Lunch&mealType=Dinner&random=True'
  ...
  getAllRecipes(): Observable<Recipe[]> {
    return this.http.get<any>(this.apiURL).pipe(
      map(result => result.hits.map((result: { recipe: any; }) => result.recipe))
    );
  }

  getOneRecipe(id: string | number): Observable<Recipe> {
    return this.http.get<Recipe>(this.apiURL + self + id)
  }
}
